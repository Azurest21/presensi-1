  ...
  <!-- Content Wrapper. Contains page content -->
    ...
    <section id="hero" class="d-flex align-items-center justify-content-center">
      <div class="card" data-aos="fade-up">
        <form action="{{ route('simpan-krs') }}" method="POST">
          {{ csrf_field() }}
          <div class="card-body row">
            <div class="col-5 text-center d-flex align-items-center justify-content-center">
              <div class="">
                <p class="lead mb-8"><strong>INPUT KRS</strong></p>
              </div>
            </div>
            <div class="col-7 align-items-center justify-content-center">
              <div class="form-group">
                  <label for="sks">Mata Kuliah</label>
                  <select class="form-control"  id="matkul_id" name="matkul_id" request>
                    <option  hidden> Pilih Matkul</option>
                    @foreach($matkul as $matkul)
                    <option value="{{ $matkul->id }}">{{ $matkul->matkul }} ({{ $matkul->dosen->namadosen}})</option>
                    @endforeach
                    </select>
              </div>
              <div>
